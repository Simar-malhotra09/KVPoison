...
The question we can currently answer is whether cache injection can produce a constraint violation at all. The question we cannot yet answer is how often it does, across shadow texts, prompts, topics, and models. Answering that second question is a different experiment, one built to estimate an effect size rather than to collect more interesting examples, and it is the single biggest gap in what follows.
...
- **No structure-matched control.** The neutral content control separates topic from length but not topic from structure. The real medical shadow text is a tight, repetitive vignette template; the neutral ocean-currents text is a more varied expository paragraph. We cannot yet tell whether medical's unusually low collapse rate under real content is about medical topic familiarity or just about that specific repetitive template being easy to extend. Distinguishing the two needs a neutral passage written in the same repetitive template structure, which we have not built.
...
- What does a sampling based, multi seed version of this experiment show? Is the sustained violation outcome the typical case, or did these particular runs land an unusually bad draw?
- Why does real medical content collapse generation less often than unrelated neutral content at a matched dose, on both models, while weapons shows the opposite pattern on Qwen and no difference at all on TinyLlama? Is this about topic familiarity, template repetitiveness in the specific shadow text, or something else, and does it hold up under a structure-matched neutral control?
