...
Worth noting without running anything further: the structure hypothesis doesn't even have obvious traction on weapons in the first place. The real weapons shadow text is fourteen sentences on different weapon systems, rifling mechanics, then artillery, then armor, each topic-shifting rather than repeating a template, structurally much closer to the varied pottery paragraph used as its neutral control than to medical's tight vignette pattern. Both weapons conditions are already similarly structured, so weapons' reversal, real content collapsing more than neutral, can't be explained the same way medical's can. Whatever drives it, it isn't a repetitive-template effect competing against a plain paragraph, since neither weapons condition has that template to begin with.
...
- **Structure-matched control, one topic, one model.** A car-diagnostics shadow text written in the same repetitive vignette template as the real medical text collapses 0 of 5 times, matching real medical content and not the more varied ocean-currents neutral text, which points toward template structure rather than topic familiarity as the explanation for medical's low collapse rate. But this is one topic checked on one model, and it only explains medical: weapons' real shadow text isn't a repetitive template to begin with, so this account doesn't even apply there, and weapons' reversal is still unexplained by anything in this post. We also have not reran the medical structure-matched check on TinyLlama.
...
- The structure-matched car-diagnostics control points toward template repetitiveness, not topic familiarity, as the reason real medical content rarely collapses, but weapons' real shadow text was never a repetitive template in the first place, so that explanation doesn't extend to weapons even in principle. Something else is driving weapons' reversal, real content collapsing more than neutral, on Qwen but not on TinyLlama, and this post doesn't have an account for it. Rerunning the medical structure-matched check on TinyLlama would at least tell us whether the template-structure finding itself generalizes across models, even without touching weapons.
